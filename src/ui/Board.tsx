...
export function Board() {
  ...
  // preserve container height (same height + width = same fitted scale;
  // see ui/openLift.ts for why that also protects the pan/zoom mode
  // equivalence). bottom stays expressed against var(--drawer-peek), so
  // the lift COMPOSES with the peek system: a peek swap while open still
  // flows through. Cleanup restores the stylesheet insets.
  useLayoutEffect(() => {
    const el = containerRef.current;
    ...
    const apply = () => {
      ...
      // must drop the lift, not keep applying phone math to the side
      // panel.
      ...
      const lift = openLiftPx({
        ...
        // center (both layouts center at cy = 0): base board top row
        // centers at y = -3 (bottom vertex -2), the expansion's at -4.5
        // (bottom -3.5).
